...
public partial class NewHipsEntry : ContentPage
{
    ...
    public NewHipsEntry(ISQLiteDataService localData, DateTime date, WeightTable weightEntry)
    {
        InitializeComponent();
    ...
    protected override async void OnAppearing()
    {
        ...
        {
            var number = await _localData.GetHips(userID, _date);
        ...
        {
            var number = Math.Round(_weightEntry.Hips, 2);
    ...
    async void ToolbarItem_Clicked(System.Object sender, System.EventArgs e)
    {
        ...
        if (!entryExists)
        {
            await _localData.AddHipsEntry(userID, DateSelect.Date, INCHMValue);
        }
        else
        {
            await _localData.UpdateHipsEntry(userID, DateSelect.Date, INCHMValue);
        }
