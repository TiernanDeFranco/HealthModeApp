...
public partial class NewCalvesEntry : ContentPage
{
    ...
    public NewCalvesEntry(ISQLiteDataService localData, DateTime date, WeightTable weightEntry)
    {
        InitializeComponent();
    ...
    protected override async void OnAppearing()
    {
        ...
        {
            var number = await _localData.GetCalves(userID, _date);
        ...
        {
            var number = Math.Round(_weightEntry.Calves, 2);
    ...
    async void ToolbarItem_Clicked(System.Object sender, System.EventArgs e)
    {
        ...
        if (!entryExists)
        {
            await _localData.AddCalvesEntry(userID, DateSelect.Date, INCHMValue);
        }
        else
        {
            await _localData.UpdateCalvesEntry(userID, DateSelect.Date, INCHMValue);
        }
