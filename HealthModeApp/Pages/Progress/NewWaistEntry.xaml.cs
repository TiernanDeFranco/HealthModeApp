...
public partial class NewWaistEntry : ContentPage
{
    ...
    public NewWaistEntry(ISQLiteDataService localData, DateTime date, WeightTable weightEntry)
    {
        InitializeComponent();
    ...
    protected override async void OnAppearing()
    {
        ...
        {
            var number = await _localData.GetWaist(userID, _date);
        ...
        {
            var number = Math.Round(_weightEntry.Waist, 2);
    ...
    async void ToolbarItem_Clicked(System.Object sender, System.EventArgs e)
    {
        ...
        if (!entryExists)
        {
            await _localData.AddWaistEntry(userID, DateSelect.Date, INCHMValue);
        }
        else
        {
            await _localData.UpdateWaistEntry(userID, DateSelect.Date, INCHMValue);
        }
