...
public partial class NewThighsEntry : ContentPage
{
    ...
    public NewThighsEntry(ISQLiteDataService localData, DateTime date, WeightTable weightEntry)
    {
        InitializeComponent();
    ...
    protected override async void OnAppearing()
    {
        ...
        {
            var number = await _localData.GetThighs(userID, _date);
        ...
        {
            var number = Math.Round(_weightEntry.Thighs, 2);
    ...
    async void ToolbarItem_Clicked(System.Object sender, System.EventArgs e)
    {
        ...
        if (!entryExists)
        {
            await _localData.AddThighsEntry(userID, DateSelect.Date, INCHMValue);
        }
        else
        {
            await _localData.UpdateThighsEntry(userID, DateSelect.Date, INCHMValue);
        }
